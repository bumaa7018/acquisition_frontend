...
async function listParcelsFromAcquisitions(
  params?: ParcelListParams,
): Promise<PaginatedResponse<GlobalParcel>> {
  ...
  const fetchAcquisitions = async (pageNo: number) =>
    api
      .get<PaginatedResponse<LandAcquisition>>("/land-acquisitions", {
        params: {
          page: pageNo,
          page_size: acqPageSize,
  ...
  const otherAcqPages =
    firstAcqPage.total_pages > 1
      ? await Promise.all(
          Array.from({ length: firstAcqPage.total_pages - 1 }, (_, i) =>
            fetchAcquisitions(i + 2),
          ),
        )
      : [];
  ...
  const acquisitions = [firstAcqPage, ...otherAcqPages]
    .flatMap((r) => r.data)
    .filter((acq) => {
      if (params?.acquisition_id && acq.id !== params.acquisition_id)
        return false;
      const name = params?.acquisition_name?.trim().toLowerCase();
      if (name && !(acq.acquisition_name ?? "").toLowerCase().includes(name))
        return false;
      const plan = params?.plan_code?.trim().toLowerCase();
      if (plan && !(acq.plan_code ?? "").toLowerCase().includes(plan))
        return false;
      if (
        selectedYears.size > 0 &&
        !selectedYears.has(yearFromDate(acq.start_date))
      )
        return false;
      return true;
    });

  const parcelPages = await Promise.all(
    acquisitions.map(async (acq) => {
      const res = await api
        .get<PaginatedResponse<LegacyParcel>>(
          `/land-acquisitions/${acq.id}/parcels`,
          {
            params: {
              page: 1,
              page_size: 1000,
  ...
  const filtered = parcelPages.flat().filter((parcel) => {
    if (params?.right_type && parcel.right_type !== params.right_type)
      return false;
...
export const usersApi = {
  ...
  update: (
    id: string,
    body: Partial<{ email: string; first_name: string; last_name: string }>,
  ) =>
...
export const permissionsApi = {
  list: () =>
    api.get<PaginatedResponse<Permission>>("/permissions").then((r) => r.data),
  create: (body: { name: string; description?: string }) =>
    api
      .post<ApiResponse<Permission>>("/permissions", body)
      .then((r) => r.data.data),
...
export const landApi = {
  listConstructionTypes: () =>
    api
      .get<ApiResponse<ConstructionType[]>>("/construction-types")
      .then((r) => r.data.data),
  list: (filter?: LandAcquisitionFilter) =>
    api
      .get<
        PaginatedResponse<LandAcquisition>
      >("/land-acquisitions", { params: filter })
      .then((r) => r.data),
  getById: (id: string) =>
    api
  ...
  create: (data: FormData) =>
    api
      .post<ApiResponse<LandAcquisition>>("/land-acquisitions", data, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      .then((r) => r.data.data),
  update: (id: string, data: FormData) =>
    api
      .put<ApiResponse<LandAcquisition>>(`/land-acquisitions/${id}`, data, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      .then((r) => r.data.data),
  ...
  getParcels: (
    ...
    api
      .get<
        PaginatedResponse<Parcel>
      >(`/land-acquisitions/${id}/parcels`, { params })
      .then((r) => r.data),
  getAssets: (
    id: string,
    ...
    api
      ...
      >(`/land-acquisitions/${id}/assets`, { params })
      .then((r) => r.data),
  createAsset: (acqId: string, body: Partial<Asset>) =>
    api
      .post<ApiResponse<Asset>>(`/land-acquisitions/${acqId}/assets`, body)
      .then((r) => r.data.data),
  updateAsset: (acqId: string, assetId: string, body: Partial<Asset>) =>
    api
      .put<
        ApiResponse<Asset>
      >(`/land-acquisitions/${acqId}/assets/${assetId}`, body)
      .then((r) => r.data.data),
  ...
  listCompensations: (acqId: string, parcelId?: string) =>
    api
      .get<ApiResponse<Compensation[]>>(
        `/land-acquisitions/${acqId}/compensations`,
        {
  ...
  createCompensation: (acqId: string, body: Partial<Compensation>) =>
    api
      ...
      >(`/land-acquisitions/${acqId}/compensations`, body)
      .then((r) => r.data.data),
  updateCompensation: (
    acqId: string,
    compId: string,
    ...
    api
      .put<
        ApiResponse<Compensation>
      >(`/land-acquisitions/${acqId}/compensations/${compId}`, body)
      .then((r) => r.data.data),
  ...
  createCompensationGrant: (
    acqId: string,
    compId: string,
    body: Partial<CompensationGrant>,
  ) =>
    api
      .post<
        ApiResponse<CompensationGrant>
      >(`/land-acquisitions/${acqId}/compensations/${compId}/grant`, body)
      .then((r) => r.data.data),
  updateCompensationGrant: (
    acqId: string,
    compId: string,
    body: Partial<CompensationGrant>,
  ) =>
  ...
  setParcelCompensation: (acqId: string, parcelId: string, paid: boolean) =>
    api
      .put(`/land-acquisitions/${acqId}/parcels/${parcelId}/compensation`, {
        paid,
      })
      .then((r) => r.data),
  getParcel: (acqId: string, parcelId: string) =>
    api
      .get<
        ApiResponse<ParcelFull>
      >(`/land-acquisitions/${acqId}/parcels/${parcelId}`)
      .then((r) => r.data.data),
  ...
  updateParcelValuation: (
    ...
    body: {
      ...
      base_price_per_ha?: number | null;
      auction_coeff?: number | null;
      auction_price?: number | null;
    },
  ) =>
    api.patch(
      `/land-acquisitions/${acqId}/parcels/${parcelId}/valuation`,
      body,
    ),
  getProgress: (id: string) =>
    api
      ...
      >(`/land-acquisitions/${id}/progress`)
      .then((r) => r.data.data),
  getAvailableStatuses: (id: string) =>
    api
      .get<
        ApiResponse<StatusOption[]>
      >(`/land-acquisitions/${id}/available-statuses`)
      .then((r) => r.data.data),
  advanceStatus: (
    id: string,
    toStatus: number,
    ...
    api
      .post<ApiResponse<LandAcquisition>>(`/land-acquisitions/${id}/advance`, {
        to_status: toStatus,
        note: note ?? "",
  ...
  updateParcelMeta: (
    ...
    parcelId: string,
    dbChanged: boolean,
    changedParcelId: string,
  ) =>
    api
      .patch(`/land-acquisitions/${acqId}/parcels/${parcelId}/meta`, {
  ...
  listDocuments: (id: string) =>
    api
      .get<ApiResponse<Document[]>>(`/land-acquisitions/${id}/documents`)
      .then((r) => r.data.data ?? []),
  uploadDocument: (id: string, file: File) => {
    const fd = new FormData();
    fd.append("file", file);
    return api
      .post<ApiResponse<Document>>(`/land-acquisitions/${id}/documents`, fd, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      .then((r) => r.data.data);
  },
  ...
  listParcelStatuses: () =>
    api
      .get<ApiResponse<ParcelStatus[]>>("/parcel-statuses")
      .then((r) => r.data.data),
};
...
export const parcelApi = {
  list: async (params?: ParcelListParams) => {
    ...
    try {
      return await api
        .get<PaginatedResponse<GlobalParcel>>("/parcels", { params })
        .then((r) => r.data);
    } catch (error) {
  ...
  listPayments: (id: string) =>
    api
      .get<ApiResponse<ParcelPayment[]>>(`/parcels/${id}/payments`)
      .then((r) => r.data.data ?? []),
  addPayment: (
    id: string,
    body: { amount: number; currency: string; paid_at?: string; note?: string },
  ) => api.post(`/parcels/${id}/payments`, body).then((r) => r.data),
  listDocuments: (id: string) =>
    api
      .get<ApiResponse<Document[]>>(`/parcels/${id}/documents`)
      .then((r) => r.data.data ?? []),
  uploadDocument: (id: string, file: File) => {
    const fd = new FormData();
    fd.append("file", file);
    return api
      .post<ApiResponse<Document>>(`/parcels/${id}/documents`, fd, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      .then((r) => r.data.data);
  },
...
export const reportApi = {
  list: (params?: ReportListParams) => {
    ...
    if (params?.acquisition_name)
      q.set("acquisition_name", params.acquisition_name);
    ...
    if (params?.compensation_type)
      q.set("compensation_type", params.compensation_type);

    const suffix = q.toString();
    return api
      .get<
        PaginatedResponse<ReportParcelRow>
      >(`/report/download${suffix ? `?${suffix}` : ""}`)
      .then((r) => r.data);
  },
};
...
export const planApi = {
  search: (code: string) =>
    api
      .get<ApiResponse<Plan>>("/plans/search", { params: { code } })
      .then((r) => r.data.data),
  suggest: (q: string) =>
    api
