...
const inputCls = 'w-full rounded-lg border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#1e1f27] px-3 py-2 text-[13px] text-slate-800 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-600 outline-none focus:border-[#02c0ce] focus:ring-2 focus:ring-[#02c0ce]/15 transition-all'
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-800 dark:text-white">Хэрэглэгчид</h1>
          <p className="text-[12px] text-slate-500 dark:text-slate-400 mt-0.5">Нийт {data?.total ?? 0} хэрэглэгч</p>
        </div>
        <button
      ...
      {showCreate && (
        <div className="ap-card p-5">
          <p className="text-[13px] font-semibold text-slate-700 dark:text-white mb-4">Шинэ хэрэглэгч</p>
          <form onSubmit={handleSubmit(d => createMutation.mutate(d))} className="grid grid-cols-2 gap-4">
            {fields.map(([field, label, type]) => (
              <div key={field}>
                <label className="block text-[12px] font-medium text-slate-600 dark:text-slate-300 mb-1.5">{label}</label>
                <input type={type} {...register(field)} className={inputCls} />
                {errors[field] && <p className="mt-1 text-[11px] text-[#f1556c]">{errors[field]?.message}</p>}
              </div>
            ))}
          ...
          <table className="w-full text-[13px]">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-[#37394d]">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-5 py-12 text-center text-[13px] text-slate-400 dark:text-slate-500">
                    <Users className="mx-auto mb-2 h-8 w-8 opacity-30" />
                    Хэрэглэгч олдсонгүй
                  </td>
                </tr>
              ) : data.data.map(user => (
                <tr key={user.id} className="hover:bg-slate-50/60 dark:hover:bg-[#252630] transition-colors">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="font-medium text-slate-800 dark:text-white">{user.first_name} {user.last_name}</p>
                    </div>
                  </td>
