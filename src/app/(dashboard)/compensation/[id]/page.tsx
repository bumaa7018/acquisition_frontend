...
import {
  ...
  MapPin, Phone, Hash, Calendar, CreditCard,
} from 'lucide-react'
import { INVOICES, STATUS_CONFIG, ISSUER, invoiceTotal, fmtMoney } from '../mock-data'
      ...
      <div className="flex flex-col gap-5">
        ...
        >
          ...
          <div className="px-10 py-8 border-b border-slate-100">
            <div className="flex items-start justify-between gap-8">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-[15px] font-bold text-slate-800 leading-tight">{ISSUER.name}</p>
                  <p className="text-[12px] text-slate-500 mt-1.5 flex items-center gap-1.5">
                    <MapPin className="h-3 w-3 shrink-0" />{ISSUER.address}
                  </p>
                  <p className="text-[12px] text-slate-500 mt-0.5 flex items-center gap-1.5">
                    <Phone className="h-3 w-3 shrink-0" />{ISSUER.phone}
                  </p>
                </div>
              </div>
              ...
              <div className="text-right shrink-0">
                <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400">НЭХЭМЖЛЭЛ</p>
                <p className="text-2xl font-black text-slate-800 mt-0.5 tabular-nums">{inv.number}</p>
                <div className="mt-2">
                  <span
          ...
          <div className="grid grid-cols-2 gap-8 px-10 py-8 border-b border-slate-100 bg-slate-50/40">
            ...
            {/* Document metadata */}
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400 mb-4">Нэхэмжлэлийн мэдээлэл</p>
              <div className="grid grid-cols-2 gap-4">
                {[
                  { icon: Hash,     label: 'Нэгж талбарын №',   value: inv.parcelId },
                  { icon: CreditCard, label: 'Нэхэмжлэлийн №',  value: inv.number },
                  ...
                  <div key={label}>
                    ...
                    <p className="text-[13px] font-semibold text-slate-700 mt-0.5 tabular-nums">{value}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Recipient */}
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400 mb-4">Нэхэмжлэл хүлээн авагч</p>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-slate-500 mt-0.5 flex items-center gap-1">
                    <MapPin className="h-3 w-3 shrink-0" />{inv.client.address}
                  </p>
                  <p className="text-[12px] text-slate-500 mt-0.5 flex items-center gap-1">
                    <Phone className="h-3 w-3 shrink-0" />{inv.client.phone}
                  </p>
                  <p className="text-[12px] text-slate-500 mt-0.5 flex items-center gap-1">
                    <CreditCard className="h-3 w-3 shrink-0" />РД: {inv.client.register}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="px-10 py-6">
            <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400 mb-4">Нэхэмжлэлийн зүйлс</p>
                ...
                <tbody className="divide-y divide-slate-50">
                  {inv.items.map((item, idx) => (
                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-4 py-3.5 text-slate-400 tabular-nums">{idx + 1}</td>
                      ...
                      <td className="px-4 py-3.5 tabular-nums text-slate-700">{item.qty.toLocaleString()}</td>
                      <td className="px-4 py-3.5 tabular-nums text-slate-700">{fmtMoney(item.unitPrice)}</td>
          ...
          <div className="grid grid-cols-2 gap-8 px-10 pb-8 items-start">
            ...
            <div>
              {inv.notes && (
                <>
                  ...
                  <p className="text-[13px] text-slate-600 leading-relaxed">{inv.notes}</p>
                </>
              )}
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <span className="text-[14px] font-bold text-slate-800">Нийт дүн</span>
          ...
          >
            <p className="text-[11px] text-slate-400">
              Энэхүү баримт бичиг нь автоматаар үүссэн бөгөөд гарын үсэг шаардахгүй.
            </p>
            <p className="text-[11px] font-semibold text-[#02c0ce] tabular-nums">{inv.number}</p>
          </div>
        </div>
      </div>
    </>
