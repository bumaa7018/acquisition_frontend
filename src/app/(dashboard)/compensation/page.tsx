...
import {
  INVOICES, STATUS_CONFIG, invoiceTotal, fmtMoney,
  type InvoiceStatus,
} from './mock-data'
...
export default function InvoicesPage() {
  ...
  const filtered = INVOICES.filter(inv => {
    ...
    const matchQ = !q || inv.number.toLowerCase().includes(q) || inv.client.name.toLowerCase().includes(q)
  ...
  const stats = {
    ...
    paidAmt: INVOICES.filter(i => i.status === 'paid').reduce((s, i) => s + invoiceTotal(i), 0),
    pendAmt: INVOICES.filter(i => i.status === 'pending').reduce((s, i) => s + invoiceTotal(i), 0),
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-800 dark:text-white">Нэхэмжлэл</h1>
          <p className="text-[12px] text-slate-500 dark:text-slate-400 mt-0.5">Газар чөлөөлтийн нэхэмжлэлийн бүртгэл</p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Нийт нэхэмжлэл',  value: stats.all,     icon: FileText,    color: '#02c0ce' },
          { label: 'Төлөгдсөн',        value: stats.paid,    sub: fmtMoney(stats.paidAmt), icon: CheckCircle2, color: '#0acf97' },
          { label: 'Хүлээгдэж буй',    value: stats.pending, sub: fmtMoney(stats.pendAmt), icon: Clock,        color: '#f9bc0b' },
          { label: 'Хугацаа хэтэрсэн', value: stats.overdue, icon: AlertCircle, color: '#f1556c' },
          ...
          <div key={label} className="ap-card px-5 py-4 flex items-center gap-4">
            ...
            <div className="min-w-0">
              <p className="text-[11px] text-slate-500 dark:text-slate-400 uppercase tracking-wide font-medium leading-tight">{label}</p>
              <p className="text-2xl font-bold tabular-nums text-slate-800 dark:text-white leading-tight mt-0.5">{value}</p>
              {sub && <p className="text-[11px] text-slate-400 dark:text-slate-500 leading-tight truncate">{sub}</p>}
            </div>
          </div>
        ))}
      </div>
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="border-b border-slate-100 dark:border-[#37394d] bg-slate-50/50 dark:bg-[#1a1d20]">
                {['Нэхэмжлэлийн №', 'Иргэн', 'Огноо', 'Дуусах огноо', 'Дүн', 'Төлөв', ''].map(h => (
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-[#37394d]">
              {rows.length === 0 ? (
                ...
                return (
                  <tr key={inv.id} className="hover:bg-slate-50/60 dark:hover:bg-[#252630] transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <div>
                        <p className="font-medium text-slate-800 dark:text-white">{inv.client.name}</p>
                        <p className="text-[11px] text-slate-400 dark:text-slate-500">{inv.parcelId}</p>
                      </div>
                    </td>
                    <td className="px-5 py-3.5 tabular-nums text-slate-600 dark:text-slate-400">{inv.date}</td>
                    <td className="px-5 py-3.5 tabular-nums text-slate-600 dark:text-slate-400">{inv.due}</td>
