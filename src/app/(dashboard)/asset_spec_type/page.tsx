...
import { assetSpecTypeApi } from "@/lib/api";
...
export default function AssetSpecTypePage() {
  ...
  const updateMutation = useMutation({
    mutationFn: () =>
      assetSpecTypeApi.update(editItem!.id, {
        code: form.code.trim(),
        name: form.name.trim(),
        sort_order: form.sort_order ? parseInt(form.sort_order) : 0,
      }),
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => assetSpecTypeApi.delete(id),
    onSuccess: () => {
  ...
  const openEdit = (item: AssetSpecType) => {
    setEditItem(item);
      ...
      {isFormOpen && (
        <div className="ap-card p-5">
          ...
          <div className="flex flex-col gap-3 max-w-md">
            <input
              placeholder="Код * (жишээ: construction_type)"
              value={form.code}
      ...
      <div className="ap-card overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <Layers className="h-10 w-10 text-slate-300 dark:text-[#37394d] mb-3" />
