  ...
  try {
    ...
    const parcelDetailResults = await Promise.all(
      parcels.map((p: AnyRow) =>
        backendFetch(
          `/api/v1/land-acquisitions/${id}/parcels/${p.id}`,
          token,
        ).catch(() => null),
      ),
    );
    ...
    const decreePart = [acq.decree_date ?? "", acq.decree_number ?? ""]
      .filter(Boolean)
      .join(" ");

    // 6. Мөр бүрийн утгыг бэлтгэх
    const allRowValues: (string | number | null)[][] = parcels.map(
      (p: AnyRow, idx: number) => {
        const detail: AnyRow = parcelDetailResults[idx]?.data?.detail ?? null;

        const parcelComps = compensations.filter(
          (c: AnyRow) => c.parcel_id === p.parcel_id,
        );
        const parcelAssets = assets.filter(
          (a: AnyRow) => a.parcel_id === p.parcel_id,
        );
        const realStateIds = new Set<string>(
          parcelAssets
            .filter((a: AnyRow) => a.asset_type === "real_state")
            .map((a: AnyRow) => a.id as string),
        );
        const propIds = new Set<string>(
          parcelAssets
            .filter((a: AnyRow) => a.asset_type === "property")
            .map((a: AnyRow) => a.id as string),
        );

        const landValue = parcelComps
          .filter((c: AnyRow) => c.target_type === "parcel")
        ...
        const realStateComp = parcelComps
          .filter(
            (c: AnyRow) =>
              c.target_type === "asset" &&
              c.asset_id &&
              realStateIds.has(c.asset_id),
          )
          .reduce((s: number, c: AnyRow) => s + (c.amount ?? 0), 0);
        const propComp = parcelComps
          .filter(
            (c: AnyRow) =>
              c.target_type === "asset" &&
              c.asset_id &&
              propIds.has(c.asset_id),
          )
          .reduce((s: number, c: AnyRow) => s + (c.amount ?? 0), 0);
        const totalComp = landValue + realStateComp + propComp;

        const remaining =
          p.remaining_area_m2 != null
            ? p.remaining_area_m2
            : (p.area_m2 ?? 0) - (p.acquisition_area_m2 ?? 0);

        const holderName = detail
          ? `${detail.holder_last_name ?? ""} ${detail.holder_name ?? ""}`.trim()
          : "";
    ...
    const safeName = (acq.acquisition_name || id).replace(
      /[/\\?%*:|"<>]/g,
      "_",
    );
