...
const LAYER_DEFS: (MapLayerDef & { defaultVisible: boolean; filter?: 'parcel' | 'acquisition' })[] = [
  ...
  { ...layerDef('v_acquisition_boundary'), defaultVisible: true, filter: 'acquisition' },
    ...
    setLayers(prev => prev.map(l => {
      ...
      if (next.visible && def && olMap.current) {
        void fitLayerToMap({
          ...
          wfsUrl: GS_WFS,
          layerId: def.id,
          cqlFilter: def.filter === 'parcel' ? parcelFilter : def.filter === 'acquisition' ? acquisitionFilter : undefined,
          padding: [60, 60, 60, 60],
          maxZoom: 18,
        })
      }
    ...
    fetch(`${GS_WFS}?${params}`)
      ...
      .then(json => {
        ...
        if (bbox?.length === 4) {
          ...
          map.getView().fit(ext, { padding: [60, 60, 60, 60], maxZoom: 18, duration: 1000 })
        }
      })
