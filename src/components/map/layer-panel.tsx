...
export default function LayerPanel({ layers, groups = [], onToggle }: LayerPanelProps) {
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: dark ? "0 4px 24px rgba(0,0,0,.45)" : "0 4px 24px rgba(0,0,0,.15)",
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          <div className="flex h-6 w-6 items-center justify-center rounded-md" style={{ background: "#02c0ce20" }}>
            <Layers className="h-3.5 w-3.5" style={{ color: "#02c0ce" }} />
          </div>
          <span className="text-[12px] font-bold uppercase tracking-wider" style={{ color: lblClr }}>
            Давхаргууд
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full text-white" style={{ background: "#02c0ce" }}>
            {visibleCount}/{layers.length}
          </span>
          <ChevronDown
            className="h-3.5 w-3.5 transition-transform"
            style={{ color: subClr, transform: collapsed ? "rotate(-90deg)" : "rotate(0deg)" }}
          />
        </div>
      </button>
      ...
      {!collapsed && (
        <div style={{ borderTop: `1px solid ${divClr}` }}>
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    boxShadow: layer.visible ? `0 0 0 1.5px ${layer.color}50` : "none",
                  }}
                />
                <button
                ...
                <button onClick={() => onToggle(layer.id)} className="shrink-0">
                  {layer.visible
          ...
          {groups.map(group => {
            ...
            return (
              <div key={group.id} style={{ borderTop: idx > 0 ? `1px solid ${divClr}` : "none" }}>
                {/* Group header row */}
                <div
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: anyVis ? `0 0 0 1.5px ${group.color}50` : "none",
                    }}
                  />
                  ...
                  {/* Eye — toggle all */}
                  <button onClick={() => toggleAllInGroup(group.id)} className="shrink-0">
                    {anyVis
                      ? <Eye    className="h-3 w-3" style={{ color: "#02c0ce" }} />
                      : <EyeOff className="h-3 w-3" style={{ color: subClr }} />
                    }
                  ...
                  <button onClick={() => toggleGroup(group.id)} className="shrink-0">
                    <ChevronDown
                      ...
                      style={{ color: subClr, transform: isExpanded ? "rotate(180deg)" : "rotate(0deg)" }}
                    />
                  </button>
                </div>
                  ...
                  <div style={{ borderTop: `1px solid ${divClr}` }}>
                    {children.map(child => (
                      <button
                        ...
                        style={{
                          background: child.visible ? `${child.color}14` : "transparent",
                          opacity: child.visible ? 1 : 0.5,
                        }}
                        onMouseEnter={e => { if (!child.visible) e.currentTarget.style.background = hover; }}
                        onMouseLeave={e => { e.currentTarget.style.background = child.visible ? `${child.color}14` : "transparent"; }}
                      >
                        <span
                          className="shrink-0 h-2.5 w-2.5 rounded-sm"
                          style={{
                            background: child.color,
                            boxShadow: child.visible ? `0 0 0 1.5px ${child.color}50` : "none",
                            opacity: child.visible ? 1 : 0.4,
                          }}
                        />
                        <span
                          className="flex-1 text-left text-[11px] font-medium truncate"
                          style={{ color: child.visible ? (dark ? "#d1d5db" : "#1e293b") : subClr }}
                        >
                          {child.label}
                        </span>
                        {child.visible
                          ? <Eye    className="h-3 w-3 shrink-0" style={{ color: "#02c0ce" }} />
                          : <EyeOff className="h-3 w-3 shrink-0" style={{ color: subClr }} />
                        }
                      </button>
                    ))}
                  </div>
          ...
          {/* Footer */}
          <div className="px-3 py-2 flex justify-between gap-2" style={{ borderTop: `1px solid ${divClr}` }}>
            <button
