...
export default function FeaturePopup({ layer, properties, position, onClose }: FeaturePopupProps) {
  const entries = Object.entries(properties).filter(([k]) => !SKIP_KEYS.has(k) && properties[k] != null)
          ...
          <dl className="space-y-1.5 text-xs">
            {entries.map(([key, value]) => (
              <div key={key} className="flex gap-2">
                <dt className="text-muted-foreground shrink-0 font-medium w-28 truncate">{key}</dt>
