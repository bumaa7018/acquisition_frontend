...
const LAYER_DEFS: { id: string; label: string; color: string }[] = [
  ...
  { id: 'v_acquisition_plan',     label: 'Чөлөөлөлтийн төлөвлөгөө', color: '#ef4444' },
...
export default function MapView() {
  ...
  const [layers, setLayers] = useState<LayerConfig[]>(
    LAYER_DEFS.map(d => ({ ...d, visible: ['v_acquisition_boundary', 'parcel'].includes(d.id) }))
  )
  ...
  useEffect(() => {
    ...
    LAYER_DEFS.forEach(d => {
      wmsRecord[d.id] = makeWmsLayer(d.id, ['v_acquisition_boundary', 'parcel'].includes(d.id))
    })
