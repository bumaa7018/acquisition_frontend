...
test("external role-ууд зөвхөн зөвшөөрөгдсөн tab-уудыг харна", () => {
  ...
  assert.equal(canViewParcelTabForActor(finance, "payment"), false);
  assert.equal(canViewParcelTabForActor(senior, "payment"), true);
});
...
test("нөхөх олговорын дэд tab харах эрхүүд зөв байна", () => {
  ...
  assert.equal(
    canViewValuationSubTabForActor(primaryProfessional, "independent"),
    true,
  );
...
test("external хэрэглэгч frontend API client-ээр зөвхөн зөвшөөрөгдсөн endpoint дуудна", () => {
  assert.equal(canCallApiEndpointForActor(mika, "GET", "/land-acquisitions"), true);
  assert.equal(
    canCallApiEndpointForActor(mika, "GET", "/land-acquisitions/acq-1/parcels"),
    true,
  );
  assert.equal(
    canCallApiEndpointForActor(
      finance,
      "GET",
      "/land-acquisitions/acq-1/parcels/parcel-1",
    ),
    true,
  );
  assert.equal(canCallApiEndpointForActor(mika, "GET", "/users"), false);
  assert.equal(canCallApiEndpointForActor(finance, "GET", "/roles"), false);
  assert.equal(canCallApiEndpointForActor(primaryProfessional, "GET", "/dashboard"), false);
  assert.equal(
    canCallApiEndpointForActor(primaryProfessional, "POST", "/land-acquisitions"),
    false,
  );
  assert.equal(
    canCallApiEndpointForActor(
      primaryProfessional,
      "PATCH",
      "/land-acquisitions/acq-1/parcels/parcel-1/independent-org",
    ),
    false,
  );
  assert.equal(
    canCallApiEndpointForActor(
      primaryProfessional,
      "POST",
      "/land-acquisitions/acq-1/assets",
    ),
    true,
  );
  assert.equal(
...
test("итгэмжлэгдсэн төлөөлөгчийн endpoint-ийн эрх зөв байна", () => {
  const repListPath = "/land-acquisitions/acq-1/parcels/parcel-1/representatives";
  const repDeletePath = "/land-acquisitions/acq-1/parcels/parcel-1/representatives/rep-uuid";

  // Дотоод хэрэглэгч бүх method ашиглаж болно
  assert.equal(canCallApiEndpointForActor(senior, "GET", repListPath), true);
  assert.equal(canCallApiEndpointForActor(senior, "POST", repListPath), true);
  assert.equal(canCallApiEndpointForActor(senior, "DELETE", repDeletePath), true);

  // Санхүүгийн мэргэжилтэн зөвхөн уншиж болно, бичих боломжгүй
  assert.equal(canCallApiEndpointForActor(finance, "GET", repListPath), true);
  assert.equal(canCallApiEndpointForActor(finance, "POST", repListPath), false);
  assert.equal(canCallApiEndpointForActor(finance, "DELETE", repDeletePath), false);

  // МИКА зөвхөн уншиж болно
  assert.equal(canCallApiEndpointForActor(mika, "GET", repListPath), true);
  assert.equal(canCallApiEndpointForActor(mika, "POST", repListPath), false);

  // Мэргэжлийн байгууллага зөвхөн уншиж болно
  assert.equal(canCallApiEndpointForActor(primaryProfessional, "GET", repListPath), true);
  assert.equal(canCallApiEndpointForActor(primaryProfessional, "POST", repListPath), false);
  assert.equal(canCallApiEndpointForActor(primaryProfessional, "DELETE", repDeletePath), false);
});
